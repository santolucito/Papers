...
Haskell provides two relevant functions for this task in the standard library - \codeinline{concatMap} which applies a function over a list and concatenates the result, and \codeinline{replicate n} which will replicate an item n times into a list.
...
Again, only a single example is needed for \ourTool/ to unambiguously find  simple solution program, \codeinline{takeWhile odd}, using functions from the standard library. 
