...
\ruzica{add here two functions in the standard library, list them and explain}

The user provides a single example, and \ourTool/ automatically synthesis a program \codeinline{concatMap (replicate 2)} that fit that example.
...
If, in addition, the user develops her own function \codeinline{dupl} which duplicates an element (see below), then \ourTool/ will provide the solution \codeinline{concatMap dupl}, as well as the solution listed above.
...
Only a single example is needed for \ourTool/ to unambiguously find the function \codeinline{takeWhile odd}.

\ruzica{ describe takeWhile and odd}
