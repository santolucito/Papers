...
It is consider a good practice in functional programming \ruzica{is there a citation?} that encourages specifying general behaviors in the form of abstract, higher-order functions, and then filling in details with first-order functions later.
In fact, many users write higher order functions first, then combine them in interesting and useful ways. \ruzica{again: a citation would be great here}
...
\ruzica{add now here a high level, three sentances describing what do we actually do: talk about two phase synthesis, parsing and using refinement types for pruning search space}


\ruzica{this part is poor}
